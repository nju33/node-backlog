...
export interface IssueModule {
  getIssues(params: BacklogIssueParams): Promise<Issue[]>;
}

export class IssueManager extends Manager implements IssueModule {
  async getIssues(params: BacklogIssueParams = {}): Promise<Issue[]> {
    try {
      const res = await this.axios.get('issues', {params});

      return res.data.map((item: any) => new Issue(item));
    } catch (err) {
